...
export class Input {
  ...
  static addEventListener (eventName, listener) {
    application.input.on(eventName, listener)
  }
  ...
  static removeEventListener (eventName, listener) {
    application.input.off(eventName, listener)
  }
  ...
  static setEnabled (enabled) {
    application.input.setEnabled(enabled)
  }
   ...
   */
  static isEnabled () {
    return application.input.isEnabled()
  }

  /**
   * Set key mapping for a classification of input devices.
   *
   * @param {InputDevice} deviceUUID UUID, from InputDevice#uuid, for the input device classification.
  ...
  static setMapping (deviceUUID, mapping) {
    if (deviceUUID) {
      const mappings = application.input.mappings
  ...
  static getMapping (deviceUUID) {
    return application.input.mappings.get(deviceUUID)
  }
   ...
   */
  static getKeyboard () {
    return application.window.keyboard
  }
   ...
   */
  static getGamepads () {
    return [ ...application.window.gamepadsById.values() ]
  }
}
