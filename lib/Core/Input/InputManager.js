...
export class InputManager extends FastEventEmitter {
  ...
  onDeviceMotion (device, axis, value, timestamp) {
    ...
    const { _enabled, mappings } = this

    if (_enabled) {
      const mapping = mappings.get(device.uuid)
    ...
    _enabled && this.emit(gAxisMove.type, gAxisMove._reset(device, timestamp, axis, value))
  }
...
let { keyup, keydown, buttonup, buttondown, axismove, connected, disconnected } = InputManager.Events

const gKeyUp = new KeyEvent(keyup, true, true)
const gKeyDown = new KeyEvent(keydown, true, true)
const gButtonUp = new DeviceButtonEvent(buttonup)
const gButtonDown = new DeviceButtonEvent(buttondown)
const gAxisMove = new DeviceAxisEvent(axismove)
const gConnected = new DeviceEvent(connected)
const gDisconnected = new DeviceEvent(disconnected)
