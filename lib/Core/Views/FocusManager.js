...
export class FocusManager {
  ...
  setFocus (view) {
    this._setFocus(view, new Navigate(Direction.RIGHT), true)
  }
      ...
      while (walker != null) {
        if (walker.focusDelegate) {
          ...
          if (command === Navigate.DONE) {
            ...
            focusChanged = this._setFocus(navigate.next, navigate, false)
...
function syncChildFocus (focused) {
  const navigate = new Navigate(Navigate.SYNC_CHILD)
