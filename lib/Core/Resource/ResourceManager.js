...
export class ResourceManager {
  ...
  acquireResourceBySource (src) {
    return this.acquireResource(getSourceId(src))
  }

  acquireFontResource ({ fontFamily, fontStyle, fontWeight, fontSize }) {
    // TODO: more sophisticated search.. style and weight are optional
    return this.acquireResource(`${fontFamily}-${fontStyle || 'normal'}-${fontWeight || 'normal'}-${fontSize}`)
  }
    ...
    while (!this._workQueue.isEmpty()) {
      ...
      try {
        if (type === LOAD) {
          ...
          console.time('attach: ' + item.id)
          item.resource._attach(devices)
          console.timeEnd('attach: ' + item.id)
          dirty = true
        }
      } catch (err) {
