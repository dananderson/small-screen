...
describe('Image', () => {
  ...
  describe('load()', () => {
    ...
    it('should load a png image from base64 encoded string', () => {
      return image.load(Buffer.from(TEST_BASE64, 'base64'), { type: SourceType.BASE64 })
        .then(() => {
    ...
    it('should NOT load an image from corrupt Base64 encoded string', () => {
      return image.load(TEST_BAD_BASE64, { type: SourceType.BASE64 })
        .then(() => {
          assert.isOk(false, 'Resolve was not expected.')
        })
        .catch((err) => {
    ...
    it('should load an SVG image from an XML string', () => {
      return image.load(TEST_SVG_XML, { type: SourceType.UTF8 })
        .then(() => {
    ...
    it('should NOT load an image from a corrupt XML string', () => {
      return image.load(TEST_BAD_SVG_XML, { type: SourceType.UTF8 })
        .then(() => {
          assert.isOk(false, 'Resolve was not expected.')
        })
        .catch((err) => {
